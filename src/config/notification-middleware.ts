...
const getFieldErrorsTosts = (fieldErrors: FieldErrorVM[]): TostMessage[] =>
  fieldErrors.map((fieldError) => {
    ...
    const fieldName = translate(
      `dashboardApp.${fieldError.objectName}.${convertedField}`
    );
    return {
      message: `Error on field "${fieldName}"`,
      key: `error.${fieldError.message}`,
      data: { fieldName },
    };
  });

export default () =>
  (next: (arg0: UnknownAction) => any) =>
      ...
      if (error.response) {
        ...
        if (response.status === 401) {
          ...
          if (problem?.fieldErrors) {
            getFieldErrorsTosts(problem.fieldErrors).forEach((message) =>
              addErrorAlert(message)
            );
          } else {
            const { error: toastError, param } = getMessageFromHeaders(
