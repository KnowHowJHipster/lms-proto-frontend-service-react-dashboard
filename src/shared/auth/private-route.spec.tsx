...
describe('private-route component', () => {
  ...
  const wrapper = (
    ...
    authentication: {
      ...
      account: { authorities: never[] } | { authorities: string[] };
    }
  ) => {
  ...
  it('Should render a route for the component provided when authenticated', () => {
    ...
    expect(container.innerHTML).eq('<div>Test</div>');
  });
  ...
  it('Should redirect when not authenticated', () => {
    ...
    expect(container.innerHTML).not.eq('<div>Test</div>');
    expect(container.innerHTML).eq('<div>Login</div>');
  });
});
...
describe('hasAnyAuthority', () => {
  ...
  it('Should return false when authorities is invalid', () => {
    expect(hasAnyAuthority(undefined, undefined)).eq(false);
    expect(hasAnyAuthority(null, [])).eq(false);
    expect(hasAnyAuthority([], [])).eq(false);
    expect(hasAnyAuthority([], [AUTHORITIES.USER])).eq(false);
  });

  it('Should return true when authorities is valid and hasAnyAuthorities is empty', () => {
    expect(hasAnyAuthority([AUTHORITIES.USER], [])).eq(true);
  });

  it('Should return true when authorities is valid and hasAnyAuthorities contains an authority', () => {
    expect(hasAnyAuthority([AUTHORITIES.USER], [AUTHORITIES.USER])).eq(true);
    expect(
      hasAnyAuthority([AUTHORITIES.USER, AUTHORITIES.ADMIN], [AUTHORITIES.USER])
    ).eq(true);
    expect(
      hasAnyAuthority(
  ...
  it('Should return false when authorities is valid and hasAnyAuthorities does not contain an authority', () => {
    expect(hasAnyAuthority([AUTHORITIES.USER], [AUTHORITIES.ADMIN])).eq(false);
    expect(
      hasAnyAuthority([AUTHORITIES.USER, AUTHORITIES.ADMIN], ['ROLE_USERSS'])
    ).eq(false);
    expect(
      hasAnyAuthority(
        [AUTHORITIES.USER, AUTHORITIES.ADMIN],
