...
describe('Authentication reducer tests', () => {
  ...
  describe('Requests', () => {
    it('should detect a request', () => {
      expect(
        authentication(undefined, { type: authenticate.pending.type })
      ).toMatchObject({
        loading: true,
      });
      expect(
        authentication(undefined, { type: getAccount.pending.type })
      ).toMatchObject({
        loading: true,
      });
    });
  ...
  describe('Failure', () => {
    it('should detect a failure on login', () => {
      ...
      const toTest = authentication(undefined, {
        type: authenticate.rejected.type,
        error,
      });
  ...
  describe('Actions', () => {
    ...
    it('dispatches LOGOUT actions', async () => {
      ...
      expect(store.getState()).eq([
        expect.any(Object),
        expect.objectContaining(logoutSession()),
      ]);
    });
    ...
    it('dispatches CLEAR_AUTH actions', async () => {
      ...
      expect(store.getState()).eq([
        expect.any(Object),
        expect.objectContaining(authError('message')),
        clearAuth(),
      ]);
    });
