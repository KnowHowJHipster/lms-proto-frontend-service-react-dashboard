...
describe('Activate reducer tests', () => {
  ...
  it('should reset', () => {
    expect(
      activate({ activationSuccess: true, activationFailure: false }, reset)
    ).toMatchObject({
      activationSuccess: false,
      activationFailure: false,
    });
  ...
  it('should detect a success', () => {
    expect(
      activate(undefined, { type: activateAction.fulfilled.type })
    ).toMatchObject({
  ...
  it('should return the same state on request', () => {
    expect(
      activate(undefined, { type: activateAction.pending.type })
    ).toMatchObject({
      activationSuccess: false,
      activationFailure: false,
    });
  ...
  it('should detect a failure', () => {
    expect(
      activate(undefined, { type: activateAction.rejected.type })
    ).toMatchObject({
      activationSuccess: false,
  ...
  it('should reset the state', () => {
    ...
    expect(
      activate({ activationSuccess: true, activationFailure: true }, reset)
    ).eq({
      ...initialState,
    });
  });
